...
#   name = c("a", "b", "c", "d", "e", "f"))
# ggplot(dob, aes(x = time, y = demand)) + geom_col()
# ggplot(dob, aes(x = factor(time), y = demand)) +
#   geom_col(fill = "lightblue", color = "orange")
# ggplot(dob, aes(x = name, y = demand)) + geom_bar(stat = "identity")
...
# ggplot(tibble(x = -4:4), aes(x = x)) +
#   stat_function(fun = \(x) x^2, geom = "line")
#   stat_function(fun = ~ .x^2, geom = "line")
...
# ggplot(rbind(cabbage_exp[1:5, ], list("c52", "d21", NA, NA, NA, NA)),
...
#   ggplot(aes(x = reorder(Abb, Change), y = Change, fill = Region)) +
#   geom_col() +
...
#   geom_col() + scale_fill_manual(values = c("#CCEEFF", "#FFDDDD"), guide = F)
...
# tibble(cabbage_exp)
# cabbage_exp |> group_by(Date) |> mutate(pweight = Weight / sum(Weight)) |>
#   ggplot(aes(x = Date, y = pweight, fill = Cultivar)) +
#   geom_col() + scale_y_continuous(labels = scales::percent)
# cabbage_exp |> ggplot(aes(x = interaction(Date, Cultivar), y = Weight)) +
#   geom_col() +
#   geom_text(aes(label = Weight), vjust = 1.5, color = "white")
# mtcars |> ggplot(aes(x = factor(mtcars$cyl))) +
#   geom_bar() +
#   geom_text(aes(label = ..count..), stat = "count", color = "black", vjust = -2.5) +
#   ylim(0, 20)
# ggplot(cabbage_exp, aes(x = interaction(Date, Cultivar), y = Weight)) +
#   geom_col() +
#   geom_text(aes(label = Weight, y = Weight + 0.1))
# cabbage_exp |> ggplot(aes(x = Date, y = Weight, fill = Cultivar)) +
#   geom_col(position = "dodge") +
#   geom_text(aes(label = Weight), vjust = 1.5, position = position_dodge(0.9),
#             color = "white")
# cabbage_exp |> group_by(Date) |>
#   arrange(Date, desc(Cultivar)) |>
#   mutate(cweight = cumsum(Weight) - Weight / 2) |>
#   ggplot(aes(x = Date, y = Weight, fill = Cultivar)) +
#   geom_col() + geom_text(aes(label = Weight, y = cweight), vjust = 1.5)
# tophitters2001 |> slice(1:25) |> select(name, lg, avg) |>
#   ggplot(aes(x = reorder(name, avg), y = avg)) + geom_point() +
...
#   facet_grid(lg ~ .)
# ggplot() + geom_point(aes(x = wt, y = mpg), data = mtcars)
# simpledat_long |> ggplot(aes(x = Aval, y = value, fill = Bval)) +
#   geom_col(position = "dodge")
# simpledat_long |> ggplot(aes(x = Bval, y = value, fill = Aval)) +
#   geom_col(position = "dodge")
# simpledat_long |> ggplot(aes(x = Aval, y = value, group = Bval, color = Bval)) +
#   geom_line()
# d <- data.frame(
#   xval = 1:4,
#   yval = c(3, 5, 6, 9),
#   group = c("A","B","A","B"))
# d |> ggplot(aes(x = xval, y = yval)) + geom_point(aes(color = group), size = 3) +
#   scale_color_manual(values = c("orange", "forestgreen"))
# d |> ggplot(aes(x = xval, y = yval, color = group)) + geom_point()
# d |> ggplot(aes(x = xval, y = yval)) + geom_point(color = "blue") +
