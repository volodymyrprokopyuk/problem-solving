...
Term 1
: The white-space property is used to specify how whitespace is handled inside an
  element that contains text. The default value is normal. With this value, sequential
  whitespace characters are collapsed. If the text content exceeds the width of its
  container, it will be wrapped to the next line.
...
:::
:::{.content1}
The white-space property is used to specify how whitespace is handled inside an element
that contains text. The default value is normal. With this value, sequential whitespace
characters are collapsed. If the text content exceeds the width of its container, it
will be wrapped to the next line.
:::
:::

:::{.clear}
This text is <mark>highlighted with a mark</mark> <strong>strong</strong>
<em>emphasis</em> ~~strikeout~~, <u>underline</u> <del>delete</del> <ins>insert</ins>.
Text <sub>subscript</sub>. Text<sup>superscript</sup> T^super^. T~sub~
:::

This is abbreviation <abbr title="Hypertext Markup Language">HTML</abbr>

New tab link [PostgreSQL](https://postgresql.org){target="_blank"}

```{r format-code, tidy = T, tidy.opts = list(indent = 2)}
...
# kable(head(iris), caption = "Iris table head caption example",
#   col.names = gsub("[.]", " ", names(iris)),
#   align = "lccrr")

d <- cbind(x1 = runif(3), x2 = 10 ^ c(3, 5, 7), x3 = rnorm(3, 0, 1000))
d[rbind(c(1, 1), c(2, 3), c(3, 2))] <- NA
colnames(d) <- c("$\\alpha_0$", "$\\lim{\\beta}$", "$\\frac{\\lambda}{\\gamma}$")
kable(d, digits = c(5, 0, 2), format.args = list(scientific = F))
```

![Face](/home/vlad/.face){.face}

:::{.text-ellipsis}
Your design may require that text must fit within its container without overflowing or
wrapping. This can easily be accomplished by using the white-space, overflow, and
text-overflow properties together.
:::
...
You may have seen this behavior before, when you have multiple consecutive spaces in
your HTML, or line breaks, and they are ignored by the browser.
