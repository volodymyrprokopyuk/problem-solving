...
--     min(f.actual_arrival - f.actual_departure)
--         OVER (PARTITION BY f.flight_no) min_duration,
--     max(f.actual_arrival - f.actual_departure)
--         OVER (PARTITION BY f.flight_no) max_duration,
--     sum(CASE
--         WHEN ((f.actual_departure - f.scheduled_departure)
--             > interval '1 hour') THEN 1 ELSE 0
--         END) OVER (PARTITION BY f.flight_no) delayed_count,
...
--         extract(week FROM f.scheduled_departure))
-- SELECT DISTINCT
